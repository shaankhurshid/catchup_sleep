...
sleep <- fread('/mnt/ml4cvd/projects/skhurshid/catchup_sleep/catchup_sleep_seed_blank_2y_040525.csv')
setkey(sleep,sample_id)

# Format dates
for (j in (c('accel_date','blanked_date','phenotype_censor_date'))){set(sleep,j=j,value=as.Date(sleep[[j]],format='%Y-%m-%d'))}

# Init vars
out <- data.table(); n <- 1

# Set levels
sleep[,sleep_group8 := factor(sleep_group8,levels=c('Regular','Catchup','Inadequate'))]
...
# Save out
write.csv(out,file='/mnt/ml4cvd/projects/skhurshid/catchup_sleep/cox_sleep8_blanked_ref_regular_sleep_adjust.csv',row.names=F)